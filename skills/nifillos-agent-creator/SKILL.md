...
description: >
  Guides creation and maintenance of best-practice files for the Nifillos best-practices library.
  Handles format validation, cross-references, versioning, and catalog consistency.
description_es: >
  Guía la creación y mantenimiento de archivos de best-practice en la biblioteca de best-practices de Nifillos.
...
Use this workflow when creating a new best-practice file for the `_nifillos/core/best-practices/` library.

## Pre-flight Checks

1. **Scan existing best-practice files**: Read `_nifillos/core/best-practices/_catalog.yaml`. Extract `id`, `name`, `whenToUse`, `file` from each entry.
...
Add a new entry to `_nifillos/core/best-practices/_catalog.yaml` with:
...
Save to `_nifillos/core/best-practices/{id}.md`.
...
Use this workflow when updating best-practice files in the `_nifillos/core/best-practices/` library.
...
2. Remove its entry from `_nifillos/core/best-practices/_catalog.yaml`
3. Scan ALL remaining best-practice files in `_nifillos/core/best-practices/*.md`
...
This is the most impactful change — it affects how the Architect selects best practices during cuadrilla creation.
...
```
Read _nifillos/core/best-practices/_catalog.yaml
For each entry in catalog:
  1. Verify _nifillos/core/best-practices/{entry.file} exists
...
For each .md file in _nifillos/core/best-practices/ (excluding _catalog.yaml):
...
```
For each best-practice file A in _nifillos/core/best-practices/*.md:
  For each "NOT for: ... → See {id}" in A.whenToUse:
    1. Verify _nifillos/core/best-practices/{id}.md exists
