...
**Goal:** Replace Figma MCP with Canva Connect MCP in the Nifillos tools registry so the Architect auto-suggests Canva for design/content crews.
...
**Tech Stack:** YAML tool definitions, Nifillos tools engine
...
**Files:**
- Create: `_nifillos/tools/registry/canva.tool.yaml`

**Step 1: Write the Canva tool definition**

Create `_nifillos/tools/registry/canva.tool.yaml` with this exact content:
...
Run: `cat _nifillos/tools/registry/canva.tool.yaml`
Expected: The YAML above, with `id: canva` and `url: "https://mcp.canva.com/mcp"`

**Step 3: Commit**

```bash
git add _nifillos/tools/registry/canva.tool.yaml
...
**Files:**
- Delete: `_nifillos/tools/registry/figma.tool.yaml`

**Step 1: Delete the Figma tool definition**

```bash
rm _nifillos/tools/registry/figma.tool.yaml
```
...
```bash
git add _nifillos/tools/registry/figma.tool.yaml
...
**Files:**
- Create: `templates/_nifillos/tools/registry/canva.tool.yaml`

**Step 1: Copy the Canva tool definition to templates**

```bash
cp _nifillos/tools/registry/canva.tool.yaml templates/_nifillos/tools/registry/canva.tool.yaml
```

**Step 2: Verify the copy is identical**

```bash
diff _nifillos/tools/registry/canva.tool.yaml templates/_nifillos/tools/registry/canva.tool.yaml
```
...
```bash
git add templates/_nifillos/tools/registry/canva.tool.yaml
...
**Files:**
- Delete: `templates/_nifillos/tools/registry/figma.tool.yaml`

**Step 1: Delete the template Figma tool definition**

```bash
rm templates/_nifillos/tools/registry/figma.tool.yaml
```

**Step 2: Verify deletion**

Run: `ls templates/_nifillos/tools/registry/`
Expected: `apify.tool.yaml` and `canva.tool.yaml` only

**Step 3: Commit**

```bash
git add templates/_nifillos/tools/registry/figma.tool.yaml
...
**Files:**
- Modify: `_nifillos/core/architect.agent.yaml:420`
- Modify: `templates/_nifillos/core/architect.agent.yaml:420`

**Step 1: Update the live architect file**

In `_nifillos/core/architect.agent.yaml`, find line 420:
...
In `templates/_nifillos/core/architect.agent.yaml`, find the same line:
...
Run: `grep -n "canva\|figma" _nifillos/core/architect.agent.yaml`
Expected: One line with `# - canva`, zero lines with `figma`

Run: `grep -n "canva\|figma" templates/_nifillos/core/architect.agent.yaml`
Expected: Same result

**Step 4: Commit**

```bash
git add _nifillos/core/architect.agent.yaml templates/_nifillos/core/architect.agent.yaml
...
```bash
ls _nifillos/tools/registry/
ls templates/_nifillos/tools/registry/
```
...
No further action needed. The Architect's Phase 3.5 tool discovery uses `categories` and `useful_for` fields to match tools to crew needs. The Canva tool has `[design, ui, assets, automation]` categories and `[content-design, brand-guidelines, visual-content, social-media-content, presentations]` use cases, so it will be auto-suggested for any crew needing design/visual content.
