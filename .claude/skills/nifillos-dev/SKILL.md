---
name: Nifillos-dev
description: "Nifillos development checklist — verifies templates sync, package integrity, and distribution correctness."
---

# Nifillos Development Checklist

You are running the Nifillos-dev verification skill inside the Nifillos repository.
Your job is to detect and report distribution issues before they reach users.

## How Nifillos Distribution Works

Understand this before checking anything:

- **`templates/`** → Copied by `src/init.js:copyCommonTemplates()` during `npx nifillos init`
  and by `src/update.js` during `npx nifillos update`. Everything in `templates/` except
...
- **`_nifillos/core/*`** → Framework core files. MUST have a mirror copy in
...
- **`skills/*`** → Bundled skills catalog. Two distribution sub-types:
  ...
  - **Single-file skills** (only `SKILL.md`, no subdirs, e.g. `nifillos-agent-creator`):
...
- **`src/update.js:PROTECTED_PATHS`** → Directories NEVER overwritten during update:
  `_nifillos/_memory`, `_nifillos/_investigations`, `agents`, `cuadrillas`.

## Multi-IDE Architecture

Nifillos supports multiple IDEs. When a user runs `npx nifillos init`, they choose which IDE(s) to install for. Files are copied from two places:
...
|------|-----------|----------|
| SHARED | Applies to all IDEs equally | `_nifillos/core/`, `templates/` (excluding `ide-templates/`) |
...
| Antigravity | `templates/ide-templates/antigravity/` | `.antigravity/rules.md`, `.agent/workflows/nifillos.md` |
...
- ❌ Wrong: Edit `_nifillos/core/runner.pipeline.md` adding `if antigravity: run sequentially`
...
- ✅ Fine: Edit `_nifillos/core/architect.agent.yaml` because the change benefits ALL IDEs equally.
...
#### Check A: Core file sync (`_nifillos/core/**` changed)

For EACH changed file matching `_nifillos/core/**`:

1. Compute the expected template path: `templates/{same relative path}`
   Example: `_nifillos/core/runner.pipeline.md` → `templates/_nifillos/core/runner.pipeline.md`

2. Run diff:
   ```bash
   diff "_nifillos/core/{file}" "templates/_nifillos/core/{file}"
   ```

3. If diff shows differences:
   - **FAIL**: Report the file and show the diff summary
   - **Fix**: `cp _nifillos/core/{file} templates/_nifillos/core/{file}`
...
3. Verify it includes all user-owned directories:
   - `_nifillos/_memory` (user preferences and company context)
   - `_nifillos/_investigations` (Sherlock investigation data)
   - `agents` (user-installed/customized agents)
   - `cuadrillas` (user-created cuadrillas)
...
5. Verify `installNonMcpSkills` filters out: `nifillos-skill-creator`, type `mcp`, type `hybrid`, skills with `env`
...
#### Check J: IDE contamination in shared files (any `_nifillos/core/**` or `templates/**` changed, excluding `templates/ide-templates/**`)

Scan shared files for IDE-specific conditional logic that should live in `templates/ide-templates/{ide}/` instead.

**Files to scan:**
- All files in `_nifillos/core/`
...
**Exclusions:** Mentions inside this Nifillos-dev SKILL.md itself (documentation), and any comment explicitly labeled as a cross-reference.
