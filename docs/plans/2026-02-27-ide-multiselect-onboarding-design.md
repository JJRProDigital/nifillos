...
During `Nifillos init`, the user is asked which IDEs they want to install Nifillos support for. Currently, only a single IDE can be selected and only Claude Code is enabled. This change adds multi-selection and enables three new IDEs: Open Code, Codex (OpenAI), and Antigravity.
...
|-----|-------|---------------|----------|
| Claude Code | `claude-code` | `SKILL.md` + `CLAUDE.md` | `.claude/skills/Nifillos/SKILL.md` + root |
...
```
templates/
  _nifillos/                                  ← always copied (unchanged)
  cuadrillas/                                    ← always copied (unchanged)
  ide-templates/                             ← NEW — skipped by copyCommonTemplates()
    claude-code/
      CLAUDE.md                              ← MOVED from templates/CLAUDE.md
      .claude/skills/Nifillos/SKILL.md        ← MOVED from templates/.claude/
    opencode/
      AGENTS.md                              ← NEW
    codex/
...
- Title changed to `# Nifillos Instructions`
...
```
  ...
  Claude Code:  type /Nifillos to get started
