...
**Goal:** Build an on-demand skills distribution system so users can browse and install Nifillos skills from the central GitHub repository, both via CLI and from within `/Nifillos`.

**Architecture:** A `skills/` folder in the Nifillos repo hosts skill files and a `manifest.json` registry. A new `src/skills.js` module handles fetch/install/remove using Node.js native `fetch`. The CLI gains `skills list|install|remove|update` subcommands. The `SKILL.md` gains a `/Nifillos skills` command route and browser flow.
...
```markdown
# Nifillos Skills Registry

This folder contains all available Nifillos skills distributed via `npx Nifillos skills install`.
...
const MANIFEST_URL = 'https://raw.githubusercontent.com/Nifillos-ai/Nifillos/main/skills/manifest.json';
const SKILL_URL_BASE = 'https://raw.githubusercontent.com/Nifillos-ai/Nifillos/main/skills';
...
test('listInstalled excludes the built-in Nifillos skill', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'nifillos-test-'));
  try {
    const skillsDir = join(dir, '.claude', 'skills');
    await mkdir(join(skillsDir, 'Nifillos'), { recursive: true });
...
export async function listInstalled(targetDir) {
  try {
    ...
    return entries
      .filter((e) => e.isDirectory() && e.name !== 'Nifillos')
      .map((e) => e.name);
  } catch {
    return [];
  }
...
```json
  "skillsNotInitialized": "No Nifillos installation found. Run 'init' first.",
  ...
  "skillsInstallHint": "Install with: Nifillos skills install <id>",
  ...
  "skillsUnknownCommand": "Unknown skills command: {cmd}\n  Usage: Nifillos skills list|install|remove|update"
```

**Step 2: Add keys to `src/locales/pt-BR.json`**

```json
  "skillsNotInitialized": "Instalação Nifillos não encontrada. Execute 'init' primeiro.",
  ...
  "skillsInstallHint": "Instale com: Nifillos skills install <id>",
  ...
  "skillsUnknownCommand": "Comando desconhecido: {cmd}\n  Uso: Nifillos skills list|install|remove|update"
```

**Step 3: Add keys to `src/locales/es.json`**

```json
  "skillsNotInitialized": "Instalación de Nifillos no encontrada. Ejecuta 'init' primero.",
  ...
  "skillsInstallHint": "Instala con: Nifillos skills install <id>",
  "skillsInstalling": "Instalando {id}...",
  "skillsInstalled": "✅ Instalada: {id}",
...
## Task 5: Wire up `bin/Nifillos.js` with skills subcommands
...
async function runInstall(id, targetDir) {
  if (!id) {
    console.log('\n  Usage: Nifillos skills install <id>\n');
    return;
  }
...
async function runRemove(id, targetDir) {
  if (!id) {
    console.log('\n  Usage: Nifillos skills remove <id>\n');
    return;
  }
...
if (command === 'init') {
  ...
  Usage:
    ...
    npx Nifillos update                  Update Nifillos to the latest version
    npx Nifillos skills list             Browse available skills
    npx Nifillos skills install <id>     Install a skill
    npx Nifillos skills remove <id>      Remove a skill
    npx Nifillos skills update           Update all installed skills

  Learn more: https://github.com/your-org/Nifillos
  `);
...
```bash
git add src/skills-cli.js bin/Nifillos.js
...
**Files:**
- Modify: `.claude/skills/Nifillos/SKILL.md`
- Modify: `templates/.claude/skills/Nifillos/SKILL.md` (keep in sync)

**Step 1: Add skills route to the Command Routing table**

In both SKILL.md files, find the Command Routing table and add this row after the `/Nifillos tools` row:

```markdown
| `/Nifillos skills` | Load Skills Browser → Browse & install skills |
```
...
```markdown
If the user selects "More options", present a second AskUserQuestion:
- **Skills** — Browse and install skills from the Nifillos registry
- **Tools** — Manage tool integrations (MCP servers, scripts, APIs)
- **Company profile** — View or update your company information
...
```
SKILLS
  /Nifillos skills           Browse available skills
  /Nifillos skills install <id>  Install a skill
  /Nifillos skills remove <id>   Remove a skill
```
...
1. Use WebFetch to fetch `https://raw.githubusercontent.com/Nifillos-ai/Nifillos/main/skills/manifest.json`
   - If fetch fails, show: "Could not reach the skills registry. Check your connection."
2. Use Glob `.claude/skills/*/` to detect installed skills (exclude the `Nifillos` folder itself)
...
5. To install: WebFetch `https://raw.githubusercontent.com/Nifillos-ai/Nifillos/main/skills/<id>/SKILL.md`, then Write to `.claude/skills/<id>/SKILL.md`
...
```bash
git add .claude/skills/Nifillos/SKILL.md templates/.claude/skills/Nifillos/SKILL.md
git commit -m "feat: add skills browser to /Nifillos skill (menu + command route + browser flow)"
```
...
> **Note:** Before shipping, update the placeholder `Nifillos-ai/Nifillos` in `src/skills.js` and `SKILL.md` to the real GitHub owner/repo once it is public.

Files to update:
- `src/skills.js` — `REGISTRY_BASE` constant
- `.claude/skills/Nifillos/SKILL.md` — WebFetch URLs in "Loading the Skills Browser"
- `templates/.claude/skills/Nifillos/SKILL.md` — same

```bash
git add src/skills.js .claude/skills/Nifillos/SKILL.md templates/.claude/skills/Nifillos/SKILL.md
