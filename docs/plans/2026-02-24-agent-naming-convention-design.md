...
2. **First name:** Common human name in the user's Output Language (from `_nifillos/_memory/preferences.md`)
3. **Last name:** Playful reference to the agent's specialty or profession
4. **Uniqueness:** Each agent in a crew uses a different initial letter
...
The agent name language comes from `_nifillos/_memory/preferences.md → Output Language`, which is set during `npx Nifillos init`.
...
#### 2. Template agents (instagram-content crew)
...
#### 3. `cuadrilla-party.csv`
Update `name` and `displayName` columns to match new agent names.

### Not Changed
- `cuadrilla.yaml` — references file paths, not names
