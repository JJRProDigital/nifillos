...
## MCP (Playwright + Excalidraw)

This project ships **`opencode.json`** with MCP servers for OpenCode:

- **Playwright** — local via `npx @playwright/mcp` (Sherlock / browser automation), using `_nifillos/config/playwright.config.json`.
- **Excalidraw** — remote HTTP MCP at `https://mcp.excalidraw.com/mcp` for diagrams.

If both OpenCode and Codex are selected during init, `AGENTS.md` is written once; OpenCode still uses `.opencode/skills/nifillos/` and `opencode.json`.
