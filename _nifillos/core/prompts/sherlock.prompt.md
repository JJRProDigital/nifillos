...
Use this template when the Architect needs to investigate real content from reference profiles during cuadrilla creation. Sherlock is NOT a cuadrilla agent — it is a tool the Architect dispatches as a subagent during Phase 1.5 (Investigation) of the create-cuadrilla workflow.

## Purpose

When a user provides reference profile URLs during cuadrilla discovery ("follow the style of @username"), the Architect dispatches one Sherlock subagent per profile URL. Each subagent navigates to the profile using Playwright MCP, extracts real content (captions, text, slide content, video transcriptions), and produces a structured analysis of patterns found.

The investigation output feeds directly into cuadrilla data files — making agents, frameworks, quality criteria, and voice guidance grounded in real high-performing content rather than generic best practices.
...
```
cuadrillas/{cuadrilla-name}/_investigations/{username}/screenshots/{filename}.png
```
...
The `{cuadrilla-name}` value is always provided by the Architect in the Sherlock subagent prompt.
...
The browser profile is stored at `_nifillos/_browser_profile/`. This directory persists between runs so that login cookies are preserved.
...
5. Download the audio for transcription using yt-dlp:
   ```bash
   yt-dlp -x --audio-format wav -o "cuadrillas/{cuadrilla-name}/_investigations/{username}/reel-{id}.%(ext)s" "https://www.instagram.com/reel/{reel-id}/"
   ```

6. Transcribe the audio using whisper:
   ```bash
   whisper "cuadrillas/{cuadrilla-name}/_investigations/{username}/reel-{id}.wav" --model small --output_dir "cuadrillas/{cuadrilla-name}/_investigations/{username}/" --output_format txt
   ```
...
```bash
yt-dlp --write-auto-sub --sub-lang en --skip-download -o "cuadrillas/{cuadrilla-name}/_investigations/{channel}/%(id)s" "https://www.youtube.com/watch?v={video-id}"
```
...
```bash
yt-dlp -x --audio-format wav -o "cuadrillas/{cuadrilla-name}/_investigations/{channel}/%(id)s.%(ext)s" "https://www.youtube.com/watch?v={video-id}"
```

```bash
whisper "cuadrillas/{cuadrilla-name}/_investigations/{channel}/{video-id}.wav" --model small --output_dir "cuadrillas/{cuadrilla-name}/_investigations/{channel}/" --output_format txt
```
...
One file per profile, saved to `cuadrillas/{cuadrilla-name}/_investigations/{username}/raw-content.md`.
...
One file per profile, saved to `cuadrillas/{cuadrilla-name}/_investigations/{username}/pattern-analysis.md`.
...
## Recommendations for the cuadrilla

Five specific, actionable recommendations for how the cuadrilla should incorporate
...
One file per investigation (across all profiles), saved to `cuadrillas/{cuadrilla-name}/_investigations/consolidated-analysis.md`.

```markdown
# Consolidated Investigation: {cuadrilla-name}
...
This framework becomes the operational blueprint for the cuadrilla.
...
content. These become "never do" rules for the cuadrilla:
...
After the investigation completes, the Architect maps findings to cuadrilla data files during Phase 3 (Extraction). Each cuadrilla data file is enriched with real-world evidence from the investigation.

### Mapping: Investigation Output to Cuadrilla Data Files

| Investigation Section | Cuadrilla Data File | What Gets Extracted |
|---|---|---|
...
- **Ideator agent persona** (content cuadrillas): Hook templates and angle patterns from the investigation feed directly into the ideator's framework for generating viral angles.
...
3. The Architect notes the source of each data point in the cuadrilla data files: "[Source: Investigation — @username]" vs "[Source: Web research — URL]"
...
Sherlock recommends extraction settings based on the cuadrilla type:

- **Content creation cuadrillas** (writing, copywriting, posts): Recommend "Carousels + Reels" on Instagram, "Threads only" on Twitter, "Long-form posts" on LinkedIn. These content types have the highest signal for writing patterns.
- **Video cuadrillas** (YouTube, TikTok, Reels): Recommend "Reels only" on Instagram, "Most recent videos" on YouTube. Focus on video content and transcriptions.
- **Strategy/analysis cuadrillas**: Recommend "All types" with "deep dive" quantity. Maximum data for pattern analysis.
- **General cuadrillas**: Recommend "standard" quantity with "all types." Balanced approach.
