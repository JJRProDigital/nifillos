...
Nifillos is expected to grow to 100+ skills. Bundling all skills inside the npm package forces every user to download content they don't need. Skills should be distributed on-demand, outside the installed package.

## Approach

GitHub Raw Content + Static Manifest (zero infrastructure, CDN-backed, no rate limits).

Skills live in a `skills/` folder in the main Nifillos GitHub repository (not in `templates/`). A `manifest.json` lists all available skills. Users browse and install only what they want, via CLI or `/Nifillos`.
...
│   └── ...
├── templates/                        ← UNCHANGED
│   └── .claude/skills/Nifillos/SKILL.md
├── src/
│   ├── skills.js                     ← NEW — fetch/install/remove logic
│   ├── init.js
│   └── update.js
└── bin/
    └── Nifillos.js                    ← updated with new subcommands
```
...
```js
const REGISTRY_BASE = 'https://raw.githubusercontent.com/<owner>/Nifillos/main';
...
New subcommands registered in `bin/Nifillos.js`, logic in `src/skills.js`:

| Command | Action |
|---|---|
| `Nifillos skills list` | Fetch manifest, show available vs installed |
| `Nifillos skills install <id>` | Download SKILL.md → `.claude/skills/<id>/SKILL.md` |
| `Nifillos skills remove <id>` | Delete `.claude/skills/<id>/` |
| `Nifillos skills update` | Re-download all installed skills |
...
```
  ...
  Install with: Nifillos skills install <id>
```

"Installed" is detected by checking if `.claude/skills/<id>/` exists in the current directory — no tracking file needed.

## /Nifillos Skill Integration

New menu item added to the main Nifillos skill:
...
- Community skill publishing (MVP: Nifillos team only)
