...
**Goal:** Add a Sherlock content investigator that uses Playwright to extract and analyze real content from social media profiles during crew creation, producing raw content dumps and pattern analyses that enrich the Architect's crew design.

**Architecture:** Sherlock is a subagent the Architect dispatches during a new Phase 1.5 (Investigation Setup) of the create-cuadrilla workflow. One Sherlock subagent runs per profile URL in parallel. Each uses Playwright MCP for navigation and yt-dlp + Whisper for video transcription. Output is saved to `_nifillos/_investigations/{crew-code}/` as raw-content.md + pattern-analysis.md per profile, plus a consolidated-analysis.md across all profiles.
...
**Files:**
- Create: `templates/_nifillos/core/prompts/sherlock.prompt.md`
...
during crew creation (Phase 1.5).
...
The Architect uses Sherlock during create-cuadrilla Phase 1.5 when the user provides
reference profile URLs. Sherlock is NOT a crew agent — it's a tool the Architect
uses to gather intelligence for better crew design.
...
The profile is stored at `_nifillos/_browser_profile/`.
...
Save to: `_nifillos/_investigations/{crew-code}/{platform}-{username}/raw-content.md`
...
Save to: `_nifillos/_investigations/{crew-code}/{platform}-{username}/pattern-analysis.md`
...
1. {Specific recommendation for how the crew should structure content}
...
Save to: `_nifillos/_investigations/{crew-code}/consolidated-analysis.md`

Generated by the Architect after all Sherlock subagents complete.

Template:

# Consolidated Investigation: {crew-code}
Date: {YYYY-MM-DD}
...
## Recommended Framework for Crew
...
4. **domain-framework.md** — Use the Recommended Framework from consolidated analysis
   as the basis for the cuadrilla's operational framework
...
The Architect recommends based on the crew type being created:
- Carousel crew → recommend analyzing carousels + reels
- Video crew → recommend long-form videos
- Newsletter crew → recommend articles + long posts
- Generic content → recommend diverse mix
```

**Step 2: Verify the file was created correctly**

Run: `wc -l templates/_nifillos/core/prompts/sherlock.prompt.md`
Expected: 300+ lines

**Step 3: Commit**

```bash
git add templates/_nifillos/core/prompts/sherlock.prompt.md
...
The `_nifillos/_investigations/` directory needs to exist when a user initializes Nifillos.

**Files:**
- Modify: `templates/_nifillos/_investigations/.gitkeep` (create as template)
...
Expected: FAIL — `_nifillos/_investigations` directory does not exist

**Step 3: Create the template directory marker**

Create file: `templates/_nifillos/_investigations/.gitkeep`
Content: empty file
...
```bash
git add templates/_nifillos/_investigations/.gitkeep tests/init.test.js
...
This is the core integration task. The Architect's create-cuadrilla workflow needs a new phase between Discovery and Research.

**Files:**
- Modify: `templates/_nifillos/core/architect.agent.yaml` (lines 62-131 are the create-cuadrilla workflow)
...
```yaml
- When user provides reference profile URLs, dispatch Sherlock investigator subagents to extract real content patterns before designing the crew
```

**Step 2: Add Phase 1.5 to create-cuadrilla workflow**
...
```yaml
      ...
      and engagement data to make your crew smarter.
      ...
      2. **Configure investigation per platform** using AskUserQuestion:
         ...
         - Content types to analyze (multiSelect, with smart defaults based on crew type)
         - Number of contents to analyze (with recommendation)

         Smart defaults based on crew type:
         - Carousel/image crew → recommend: carousels + reels, quantity 10
         - Video crew → recommend: long videos, quantity 5
         - Newsletter/article crew → recommend: articles + posts, quantity 10
      ...
      4. **Read Sherlock instructions**:
         Read `_nifillos/core/prompts/sherlock.prompt.md` for the full investigation guide.

      5. **Create investigation directory**:
         Create `_nifillos/_investigations/{crew-code}/` via mkdir.
...
```yaml
      ...
      Read `_nifillos/_investigations/{crew-code}/consolidated-analysis.md` and
      ...
      - **Output Examples**: Use the highest-engagement real content from
        raw-content.md as the basis for output examples. Adapt to the cuadrilla's
      ...
      When investigation data is present, mark the crew as "investigation-enriched"
      in cuadrilla.yaml metadata:
      ```yaml
      investigation:
        enriched: true
        profiles_analyzed: 3
        date: 2026-02-24
        dir: _nifillos/_investigations/{crew-code}
      ```
```
...
```markdown
- `_nifillos/_investigations/` — Sherlock content investigations (profile analyses)
```

After the `_nifillos/_memory/` line.
...
```markdown
3. During crew creation, the **Sherlock** investigator can analyze reference profiles (Instagram, YouTube, Twitter/X, LinkedIn) to extract real content patterns
```
...
Add investigation reference to the Nifillos skill help.

**Files:**
- Modify: `templates/.claude/skills/Nifillos/SKILL.md`
...
```markdown
| `/Nifillos create <desc>` with URLs | Load Architect → Create Crew with Sherlock investigation |
```

In the help text section, update the EXAMPLES:

```
  /Nifillos create "Instagram carousel crew" (+ provide reference profile URLs when asked)
```

**Step 2: Commit**

```bash
git add templates/.claude/skills/Nifillos/SKILL.md
...
Read `templates/_nifillos/core/prompts/sherlock.prompt.md` and verify:
...
Read `templates/_nifillos/core/architect.agent.yaml` and verify:
...
- [ ] cuadrilla.yaml metadata for investigation-enriched crews
...
- `_nifillos/core/prompts/sherlock.prompt.md` — exists
- `_nifillos/_investigations/` — created by init
...
|--------|------|---------------------|
| Create | `templates/_nifillos/core/prompts/sherlock.prompt.md` | ~500 |
| Create | `templates/_nifillos/_investigations/.gitkeep` | 0 (empty) |
| Modify | `templates/_nifillos/core/architect.agent.yaml` | +80-100 |
| Modify | `templates/CLAUDE.md` | +3 |
| Modify | `templates/.claude/skills/Nifillos/SKILL.md` | +3 |
